//
//  RxAuthorisationDataSource.swift
//  GBShop
//
...
class RxAuthorisationDataSource: RxTableViewSectionedReloadDataSource<AuthorisationSectionModel> {
  init() {
    super.init { _, tableView, indexPath, item in
      ...
      case 4:
        return RxAuthorisationDataSource.getTextButtonCell(item: item, tableView: tableView, indexPath: indexPath)
  ...
  private static func getTextButtonCell(
    item: AuthorisationSectionModel.Item,
    tableView: UITableView,
    indexPath: IndexPath
  ) -> UITableViewCell {
    guard let item = item as? TextButtonCellModel else { return UITableViewCell() }

    let buttonCell = tableView.dequeueReusableCell(for: indexPath) as TextButtonTableViewCell
